...
public class SavingDetail extends AppCompatActivity {
    ...
    private void initData() {
        ...
        if (lang.equals("1")) {
            goal.setText(cc.changeMoneyUSDToVND(saving.getGoal()) + "đ");
            currentAmount.setText(cc.changeMoneyUSDToVND(saving.getCurrent_amount()) + "đ");
        } else {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.mnHis) {
            Intent intent = new Intent(this, SavingHis.class);
            intent.putExtra("key", key);
            intent.putExtra("saving", (Serializable) saving);
            startActivity(intent);
            SavingDetail.this.finish();
